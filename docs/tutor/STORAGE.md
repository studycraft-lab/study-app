...
The child study page links to `/study/tutor/library`, where published eligible lessons launch directly. Children can request a catalogue heading or propose a heading/page when the chapter is not yet catalogued. Active requests are idempotent per child/chapter/section (or normalised proposed heading); siblings retain separate request histories. Previously declined requests can be requested again.

The parent tutoring page includes a section catalogue and request queue. Create an exact heading and stable section key, map a broad request to it, mark preparing, then link an eligible published version. Ready cannot be toggled without a real matching publication. A published version can fulfil several children's requests; each request is linked separately. Polling while the page is visible and a refresh button provide in-app status updates. There are no email/push notifications or automatic authoring jobs.

Unavailable/archived ready links disappear with an explanation. Previously saved eligible versions still appear under saved lessons and may finish. Request creation and fulfilment recheck course family/board/grade/chapter/section in SQL; read routes scope all data to the authenticated family and child. `20260915030000_create_tutor_requests.sql` is append-only and does not change exercise data.
