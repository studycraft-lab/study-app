...
export function AppHeader({ role, childName }: { role: "parent" | "child"; childName?: string }) {
  ...
  return <header className="app-header"><Link className="brand" href="/"><span className="brand-mark">S</span><span>StudyCraft</span></Link><nav>{role === "parent" ? <><Link href="/parent/library">Content</Link><Link href="/parent/family">Children & progress</Link><Link href="/parent/family/session">Review sessions</Link></> : <><Link href="/study/dashboard">Progress</Link><Link href="/study">Study</Link></>}<span className="role-label">{role === "parent" ? "Parent" : childName}</span><button className="button-quiet" onClick={() => leave(false)}>Switch user</button><button className="button-quiet" onClick={() => leave(true)}>Logout</button></nav></header>;
}
