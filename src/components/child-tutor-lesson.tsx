...
export function ChildTutorLesson() {
  ...
  return <main className="study-shell"><AppHeader role="child" /><Link href="/study">Back to study</Link><h1>Learn one section</h1>{error && <p role="alert">{error}</p>}{session ? <><p>Welcome back to this teaching step. Your completed checkpoints are saved.</p><TutorPlayer key={generation} pack={session.pack} progressId={session.progress.id} initialState={session.progress.state} onReload={() => void load().catch(e => setError(e.message))} onCommand={async (command: TutorCommand) => {
