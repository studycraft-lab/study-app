...
type Progress = { child: Child; history: { summary: { completedSessions: number; attempts: number; accuracy: number; mastery: number; dueReview: number }; sessions: { id: string; status: string; startedAt: string; totalQuestions: number; attempts: { correct: boolean; earned_marks: number; max_marks: number }[] }[] } };
...
export function ParentFamily() {
  ...
  return <main className="parent-shell">
    ...
    {!workspace ? <p className="study-loading">Opening the family workspace…</p> : <>
      ...
      {progress.length > 0 && <section className="library-section"><h2>Learning activity</h2><div className="parent-progress-grid">{progress.map(({ child: item, history: { summary, sessions } }) => <article key={item.id} className="parent-progress-card"><header><div><h3>{item.displayName}</h3><span>{item.board} · Grade {item.grade}</span></div><strong>{summary.accuracy}% accuracy <span className="metric-info" tabIndex={0} role="img" aria-label="Accuracy is marks earned divided by marks possible across all attempts." title="Marks earned divided by marks possible across all attempts.">i</span></strong></header><div className="parent-progress-stats"><span><strong>{summary.completedSessions}</strong> sessions</span><span><strong>{summary.mastery}%</strong> mastery <span className="metric-info" tabIndex={0} role="img" aria-label="Mastery uses the latest score for each attempted question." title="Latest score for each attempted question.">i</span></span><span><strong>{summary.dueReview}</strong> due <span className="metric-info" tabIndex={0} role="img" aria-label="Due questions are scheduled for spaced practice today." title="Scheduled for spaced practice today.">i</span></span></div>{sessions.length > 0 ? <div className="parent-session-list">{sessions.map((session) => { const earned = session.attempts.reduce((sum, attempt) => sum + attempt.earned_marks, 0); const total = session.attempts.reduce((sum, attempt) => sum + attempt.max_marks, 0); const confirming = confirmDelete === session.id; return <div key={session.id}><span>{new Date(session.startedAt).toLocaleDateString()}</span><strong>{earned}/{total || 0} marks</strong><small>{session.attempts.length} of {session.totalQuestions} answered · {session.status === "completed" ? "Completed" : "Not finished"}</small><div className="session-delete">{confirming ? <><span>This permanently removes the session and its answers.</span><button className="button-danger" disabled={busy} onClick={() => deleteSession(item.id, session.id)}>Delete permanently</button><button className="button-quiet" disabled={busy} onClick={() => setConfirmDelete("")}>Cancel</button></> : <button className="button-quiet" onClick={() => setConfirmDelete(session.id)}>Delete session</button>}</div></div>; })}</div> : <p className="empty-state">No study sessions yet.</p>}</article>)}</div></section>}
    </>}
  </main>;
}
