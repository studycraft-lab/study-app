...
export function StudyExperience() {
  ...
  const revealedCount = Object.values(statuses).filter((status) => status !== "pending").length;
  ...
  return <main className={`study-shell${phase === "session" ? " is-focus-mode" : ""}${phase === "summary" ? " is-finale-mode" : ""}`}>
    ...
    {phase === "session" && current && <section className="quiz-stage">
      ...
      <div className="focus-progress-copy"><span>{reviewingId ? "Reviewing your answer" : `Star ${currentIndex + 1} of ${questions.length}`}</span><strong>{revealedCount} revealed</strong></div>
      ...
      <article className="question-card" key={`${current.id}-${shownFeedback ? "revealed" : "answer"}`}><div className="question-meta"><span>Question {currentIndex + 1} · {current.type.replaceAll("_", " ")}</span><span>{current.marks} {current.marks === 1 ? "mark" : "marks"}</span></div><h1>{current.prompt}</h1>
        ...
        {!shownFeedback ? <div className="question-actions"><button className="answer-submit" aria-label="Submit answer" onClick={checkAnswer} disabled={!hasResponse(current, response) || busy}>{busy ? "Revealing…" : "Reveal answer"}<span aria-hidden="true">✦</span></button><button className="button-quiet" onClick={dontKnow} disabled={busy}>I don’t know yet</button></div> : <div className={`answer-feedback ${shownFeedback.reviewRequired ? "is-review" : shownFeedback.correct ? "is-correct" : shownFeedback.earnedMarks > 0 ? "is-partial" : "is-wrong"}`} role="status"><div className="feedback-verdict"><span className="verdict-orbit" aria-hidden="true"><i>✦</i></span><div><span className="feedback-kicker">{shownFeedback.reviewRequired ? "A grown-up will take a look" : shownFeedback.correct ? "A new star is shining" : shownFeedback.earnedMarks > 0 ? "Your thinking is taking shape" : "This star is ready to revisit"}</span><h2>{shownFeedback.reviewRequired ? `? Parent review needed — ${shownFeedback.earnedMarks}/${current.marks} provisional marks` : shownFeedback.correct ? "✓ Correct" : shownFeedback.earnedMarks > 0 ? `½ Partly correct — ${shownFeedback.earnedMarks}/${current.marks} marks` : "× Incorrect"}</h2></div></div>{!shownFeedback.correct && <p className="expected-answer"><strong>Expected:</strong> {shownFeedback.expectedAnswer}</p>}<p className="feedback-explanation">{shownFeedback.explanation}</p>{shownFeedback.coveredPoints?.length ? <div className="rubric-feedback"><strong>Points covered</strong><ul>{shownFeedback.coveredPoints.map((point) => <li key={point}>{point}</li>)}</ul></div> : null}{shownFeedback.partialPoints?.length ? <div className="rubric-feedback"><strong>Partly covered</strong><ul>{shownFeedback.partialPoints.map((point) => <li key={point}>{point}</li>)}</ul></div> : null}{shownFeedback.missingPoints?.length ? <div className="rubric-feedback"><strong>Points to add</strong><ul>{shownFeedback.missingPoints.map((point) => <li key={point}>{point}</li>)}</ul></div> : null}{shownFeedback.sourcePages.length > 0 && <p className="source-cite source-seal"><span aria-hidden="true">▤</span> Textbook {shownFeedback.sourcePages.map((page) => `Page ${page}`).join(", ")}</p>}<div className="feedback-next">{reviewingId ? <button onClick={closeReview}>{reviewOrigin === "summary" ? "Back to results" : "Back to current question"}</button> : <button aria-label={queue.length === 1 ? "See results" : "Next question"} onClick={() => advance(queue.slice(1))}>{queue.length === 1 ? "Reveal my constellation" : "Next star"}<span aria-hidden="true">→</span></button>}</div></div>}
      </article>
    </section>}

    {phase === "summary" && <section className="summary-card constellation-finale"><div className="finale-copy"><p className="eyebrow">Constellation revealed</p><span className="chapter-stamp" aria-hidden="true">✦<small>Session<br />complete</small></span><h1>{correctCount === questions.length ? "Every star is shining." : "Your knowledge is taking shape."}</h1><p>You revealed all {questions.length} stars and made {correctCount} shine brightly. The others are saved for another look.</p></div><div className="finale-sky"><MasteryConstellation label={`Completed session constellation: ${correctCount} correct out of ${questions.length}`} statuses={constellationStatuses(questions, statuses)} /><span>Built one answer at a time</span></div><div className="summary-counts"><div><strong>{correctCount}</strong><span>Shining</span></div><div><strong>{Object.values(statuses).filter((value) => value === "partial").length}</strong><span>Taking shape</span></div><div><strong>{Object.values(statuses).filter((value) => value === "review").length}</strong><span>Needs review</span></div><div><strong>{Object.values(statuses).filter((value) => value === "incorrect").length}</strong><span>Revisit</span></div></div><div className="answer-review"><h2>Explore your answers</h2><div className="progress-rail">{questions.map((question, index) => feedbackByQuestion[question.id] && <button type="button" key={question.id} className={`progress-step is-${statuses[question.id]}`} aria-label={`Review question ${index + 1}: ${statuses[question.id]}`} onClick={() => openReview(question.id)}>{statuses[question.id] === "correct" ? "✓" : statuses[question.id] === "partial" ? "½" : statuses[question.id] === "review" ? "?" : "×"}</button>)}</div></div><div className="question-actions finale-actions">{(Object.values(statuses).includes("incorrect") || Object.values(statuses).includes("partial")) && <button onClick={reviewMistakes}>Review mistakes</button>}<button className="button-secondary" onClick={() => setPhase("library")}>Return to my study space</button></div></section>}
