...
export function StudyExperience({ tutorEnabled = false }: { tutorEnabled?: boolean }) {
  ...
  return <main className={`study-shell${phase === "session" ? " is-focus-mode" : ""}${phase === "summary" ? " is-finale-mode" : ""}`}>
    ...
    {phase === "library" && <section className="chapter-picker">
      ...
      {!selectedSubject ? <div className="subject-grid">{subjects.map((subject) => { const available = chapters.filter((chapter) => chapter.subject === subject); const covered = available.reduce((sum, chapter) => sum + (chapter.correctEver ?? 0), 0); const total = available.reduce((sum, chapter) => sum + chapter.questionCount, 0); const pythonActivities = subject === "Computer Studies" && hasBuiltinComputer; return <button type="button" className="subject-card" data-subject={subject.toLowerCase()} key={subject} onClick={() => setSelectedSubject(subject)}><span className="subject-card-kicker">Subject</span><h2>{subject}</h2><p>{available.length} {available.length === 1 ? "chapter" : "chapters"}{pythonActivities ? " · 2 Python activities" : ""}</p><span>{total > 0 ? `${covered} of ${total} ${pythonActivities ? "chapter " : ""}questions answered correctly` : pythonActivities ? "Python lesson and practice ready" : "0 of 0 questions answered correctly"} <b aria-hidden="true">→</b></span></button>; })}</div> : <><button type="button" className="subject-back" onClick={() => setSelectedSubject("")}>← All subjects</button><div className="chapter-grid">{subjectChapters.map((chapter) => { const covered = chapter.correctEver ?? 0; const coverage = chapter.questionCount ? Math.round((covered / chapter.questionCount) * 100) : 0; const exerciseSize = Math.min(QUESTIONS_PER_EXERCISE, chapter.questionCount); return <div className="chapter-with-tutor" key={chapter.id}><button className="chapter-card" data-subject={chapter.subject.toLowerCase()} key={chapter.id} onClick={() => start(chapter)} disabled={busy} aria-label={`Study ${chapter.chapterTitle}`}><span className="chapter-card-art"><span className="chapter-orbit" aria-hidden="true" /><MasteryConstellation label={`${chapter.chapterTitle}: ${coverage}% coverage`} progress={coverage} /></span><span className="chapter-card-body"><h2>{chapter.chapterNumber ? `${chapter.chapterNumber}. ` : ""}{chapter.chapterTitle}</h2><span className="chapter-card-coverage"><span className="chapter-card-coverage-label"><span>{covered} of {chapter.questionCount} answered correctly</span>{chapter.fullCoverage && <em>Complete</em>}</span><span className="chapter-card-progress" role="progressbar" aria-label="Chapter coverage" aria-valuemin={0} aria-valuemax={100} aria-valuenow={coverage}><span style={{ width: `${coverage}%` }} /></span></span><span className="chapter-card-action">Start {exerciseSize}-question exercise <b aria-hidden="true">→</b></span></span></button>{tutorEnabled && chapter.tutorChapterId && <Link className="chapter-tutor-link" href={`/study/tutor/library?chapter=${chapter.tutorChapterId}`}>Tutoring lessons for {chapter.chapterTitle}<span aria-hidden="true">→</span></Link>}</div>; })}{selectedSubject === "Computer Studies" && hasBuiltinComputer && PYTHON_ACTIVITIES.map((activity) => <Link className="chapter-card builtin-chapter-card" href={activity.href} aria-label={activity.title} key={activity.href}><span className="chapter-card-art"><span className="chapter-orbit" aria-hidden="true" /><span className="builtin-card-glyph" aria-hidden="true">{activity.glyph}</span></span><span className="chapter-card-body"><span className="chapter-card-subject">Python {activity.kind.toLowerCase()}</span><h2>{activity.title}</h2><span className="builtin-card-description">{activity.description}</span><span className="chapter-card-action">{activity.action} <b aria-hidden="true">→</b></span></span></Link>)}</div></>}
