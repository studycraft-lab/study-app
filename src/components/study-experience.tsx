...
export function StudyExperience() {
  ...
  return <main className="study-shell">
    ...
    {phase === "library" && <section className="chapter-picker">
      ...
      {chapters.length === 0 ? <div className="empty-study"><p>No chapters are ready yet.</p><Link href="/parent/library">Import a question bank</Link></div> : <div className="chapter-grid">{chapters.map((chapter) => <button className="chapter-card" key={chapter.id} onClick={() => start(chapter)} disabled={busy} aria-label={`Study ${chapter.chapterTitle}`}><span className="chapter-card-topline"><span>{chapter.board} · Grade {chapter.grade}</span><strong>{chapter.subject}</strong></span><h2>{chapter.chapterNumber ? `${chapter.chapterNumber}. ` : ""}{chapter.chapterTitle}</h2><span className="chapter-card-details"><span><b>{chapter.correctEver ?? 0} of {chapter.questionCount}</b><small>Questions covered</small></span><span><b>{Math.min(QUESTIONS_PER_EXERCISE, chapter.questionCount)}</b><small>Questions this exercise</small></span></span>{chapter.fullCoverage && <em className="coverage-badge">✓ Full coverage</em>}</button>)}</div>}
