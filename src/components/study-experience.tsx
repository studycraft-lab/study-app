...
type Chapter = { id: string; tutorChapterId?: string; board: string; grade: number; subject: string; chapterNumber?: number | null; chapterTitle: string; questionCount: number; correctEver?: number; coveragePercent?: number; fullCoverage?: boolean };
type Child = { id: string; displayName: string; board: string; grade: number };
type Option = { id: string; text: string };
type Question = { id: string; type: string; prompt: string; marks: number; response: { options?: Option[]; left?: Option[]; right?: Option[]; requiresCorrectionWhenFalse?: boolean; editor?: string } };
...
export function StudyExperience({ tutorEnabled = false }: { tutorEnabled?: boolean }) {
  ...
  async function returnToLibrary() {
    ...
    try {
      ...
      if (!result.ok) throw new Error(body.error ?? "Could not refresh chapter progress.");
      setChapters(body.chapters ?? []); setPhase("library");
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not refresh chapter progress."); }
  ...
  const hasBuiltinComputer = child?.grade === 6;
  const subjects = [...new Set([...chapters.map((chapter) => chapter.subject), ...(hasBuiltinComputer ? ["Computer Studies"] : [])])].sort((a, b) => a.localeCompare(b));
  const subjectChapters = chapters.filter((chapter) => chapter.subject === selectedSubject);
  const resumableSessions = (history?.sessions ?? []).filter((item) => item.resumable && (!selectedSubject || item.subject === selectedSubject)).slice(0, 3);
  ...
  return <main className={`study-shell${phase === "session" ? " is-focus-mode" : ""}${phase === "summary" ? " is-finale-mode" : ""}`}>
    ...
    {phase === "library" && <section className="chapter-picker">
      <div className="chapter-picker-heading"><p className="eyebrow">{selectedSubject ? "Choose a chapter" : "Choose a subject"}</p><h1>{selectedSubject || "What would you like to study?"}</h1><p>{selectedSubject ? (tutorEnabled ? "Choose a chapter to practise or explore its tutoring lessons." : "Choose a chapter to start a focused practice session.") : "Pick a subject first. Your chapters and progress stay neatly together."}</p></div>
      {!selectedSubject ? <div className="subject-grid">{subjects.map((subject) => { const available = chapters.filter((chapter) => chapter.subject === subject); const covered = available.reduce((sum, chapter) => sum + (chapter.correctEver ?? 0), 0); const total = available.reduce((sum, chapter) => sum + chapter.questionCount, 0); return <button type="button" className="subject-card" data-subject={subject.toLowerCase()} key={subject} onClick={() => setSelectedSubject(subject)}><span className="subject-card-kicker">Subject</span><h2>{subject}</h2><p>{subject === "Computer Studies" && hasBuiltinComputer ? `${available.length + 2} modules` : `${available.length} ${available.length === 1 ? "chapter" : "chapters"}`}</p><span>{subject === "Computer Studies" && hasBuiltinComputer ? "Learn conditionals and practise exam programs" : `${covered} of ${total} questions answered correctly`} <b aria-hidden="true">→</b></span></button>; })}</div> : <><button type="button" className="subject-back" onClick={() => setSelectedSubject("")}>← All subjects</button><div className="chapter-grid">{selectedSubject === "Computer Studies" && hasBuiltinComputer && <><Link className="builtin-chapter-card" href="/study/python" aria-label="Conditional Statements"><span className="chapter-card-subject">Python lesson</span><h2>Conditional Statements</h2><p>Learn if, if…else, and if…elif…else with examples and quick checks.</p><span className="chapter-card-action">Open lesson <b aria-hidden="true">→</b></span></Link><Link className="builtin-chapter-card" href="/study/python/practice" aria-label="Python Programming"><span className="chapter-card-subject">Exam practice</span><h2>Python Programming</h2><p>Write 5- and 7-mark programs, trace output, and practise earlier Python topics.</p><span className="chapter-card-action">Open 58 questions <b aria-hidden="true">→</b></span></Link></>}{subjectChapters.map((chapter) => { const covered = chapter.correctEver ?? 0; const coverage = chapter.questionCount ? Math.round((covered / chapter.questionCount) * 100) : 0; const exerciseSize = Math.min(QUESTIONS_PER_EXERCISE, chapter.questionCount); return <div className="chapter-with-tutor" key={chapter.id}><button className="chapter-card" data-subject={chapter.subject.toLowerCase()} key={chapter.id} onClick={() => start(chapter)} disabled={busy} aria-label={`Study ${chapter.chapterTitle}`}><span className="chapter-card-art"><span className="chapter-orbit" aria-hidden="true" /><MasteryConstellation label={`${chapter.chapterTitle}: ${coverage}% coverage`} progress={coverage} /></span><span className="chapter-card-body"><h2>{chapter.chapterNumber ? `${chapter.chapterNumber}. ` : ""}{chapter.chapterTitle}</h2><span className="chapter-card-coverage"><span className="chapter-card-coverage-label"><span>{covered} of {chapter.questionCount} answered correctly</span>{chapter.fullCoverage && <em>Complete</em>}</span><span className="chapter-card-progress" role="progressbar" aria-label="Chapter coverage" aria-valuemin={0} aria-valuemax={100} aria-valuenow={coverage}><span style={{ width: `${coverage}%` }} /></span></span><span className="chapter-card-action">Start {exerciseSize}-question exercise <b aria-hidden="true">→</b></span></span></button>{tutorEnabled && chapter.tutorChapterId && <Link className="chapter-tutor-link" href={`/study/tutor/library?chapter=${chapter.tutorChapterId}`}>Tutoring lessons for {chapter.chapterTitle}<span aria-hidden="true">→</span></Link>}</div>; })}</div></>}
      {resumableSessions.length > 0 && <div className="recent-sessions"><h2>Continue studying</h2>{resumableSessions.map((item) => { const marks = sessionMarks(item); return <article key={item.id} className="recent-session-row"><span>{item.subject} · {item.chapterTitle}</span><strong>{marks.earned}/{marks.possible || 0} marks</strong><small>{item.attempts.length} of {item.totalQuestions} answered · Unfinished</small><div><button onClick={() => resume(item)} disabled={busy}>Resume session</button></div></article>; })}</div>}
