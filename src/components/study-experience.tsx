...
export function StudyExperience() {
  ...
  return <main className="study-shell">
    ...
    {phase === "library" && <section className="chapter-picker">
      ...
      {chapters.length === 0 ? <div className="empty-study"><p>No chapters are ready yet.</p><Link href="/parent/library">Import a question bank</Link></div> : <div className="chapter-grid">{chapters.map((chapter) => { const covered = chapter.correctEver ?? 0; const coverage = chapter.questionCount ? Math.round((covered / chapter.questionCount) * 100) : 0; const exerciseSize = Math.min(QUESTIONS_PER_EXERCISE, chapter.questionCount); return <button className="chapter-card" key={chapter.id} onClick={() => start(chapter)} disabled={busy} aria-label={`Study ${chapter.chapterTitle}`}><strong className="chapter-card-subject">{chapter.subject}</strong><h2>{chapter.chapterNumber ? `${chapter.chapterNumber}. ` : ""}{chapter.chapterTitle}</h2><span className="chapter-card-coverage"><span className="chapter-card-coverage-label"><span>{covered} of {chapter.questionCount} covered</span>{chapter.fullCoverage && <em>Complete</em>}</span><span className="chapter-card-progress" role="progressbar" aria-label="Chapter coverage" aria-valuemin={0} aria-valuemax={100} aria-valuenow={coverage}><span style={{ width: `${coverage}%` }} /></span></span><span className="chapter-card-action">Start {exerciseSize}-question exercise <b aria-hidden="true">→</b></span></button>; })}</div>}
    ...
    {phase === "session" && current && <section className="quiz-stage">
      ...
      <article className="question-card"><div className="question-meta"><span>{current.type.replaceAll("_", " ")}</span><span>{current.marks} {current.marks === 1 ? "mark" : "marks"}</span></div><h1>{current.prompt}</h1>
        ...
        {!shownFeedback ? <div className="question-actions"><button onClick={checkAnswer} disabled={!hasResponse(current, response) || busy}>{busy ? "Submitting…" : "Submit answer"}</button><button className="button-quiet" onClick={dontKnow} disabled={busy}>I don’t know</button></div> : <div className={`answer-feedback ${shownFeedback.reviewRequired ? "is-review" : shownFeedback.correct ? "is-correct" : shownFeedback.earnedMarks > 0 ? "is-partial" : "is-wrong"}`} role="status"><h2>{shownFeedback.reviewRequired ? `? Parent review needed — ${shownFeedback.earnedMarks}/${current.marks} provisional marks` : shownFeedback.correct ? "✓ Correct" : shownFeedback.earnedMarks > 0 ? `½ Partly correct — ${shownFeedback.earnedMarks}/${current.marks} marks` : "× Incorrect"}</h2>{!shownFeedback.correct && <p><strong>Expected:</strong> {shownFeedback.expectedAnswer}</p>}<p>{shownFeedback.explanation}</p>{shownFeedback.coveredPoints?.length ? <div className="rubric-feedback"><strong>Points covered</strong><ul>{shownFeedback.coveredPoints.map((point) => <li key={point}>{point}</li>)}</ul></div> : null}{shownFeedback.partialPoints?.length ? <div className="rubric-feedback"><strong>Partly covered</strong><ul>{shownFeedback.partialPoints.map((point) => <li key={point}>{point}</li>)}</ul></div> : null}{shownFeedback.missingPoints?.length ? <div className="rubric-feedback"><strong>Points to add</strong><ul>{shownFeedback.missingPoints.map((point) => <li key={point}>{point}</li>)}</ul></div> : null}{shownFeedback.sourcePages.length > 0 && <p className="source-cite">Textbook {shownFeedback.sourcePages.map((page) => `Page ${page}`).join(", ")}</p>}{reviewingId ? <button onClick={closeReview}>{reviewOrigin === "summary" ? "Back to results" : "Back to current question"}</button> : <button onClick={() => advance(queue.slice(1))}>{queue.length === 1 ? "See results" : "Next question"}</button>}</div>}
      </article>
    </section>}

    {phase === "summary" && <section className="summary-card"><p className="eyebrow">Session complete</p><h1>Nice work showing up.</h1><div className="summary-counts"><div><strong>{Object.values(statuses).filter((value) => value === "correct").length}</strong><span>Correct</span></div><div><strong>{Object.values(statuses).filter((value) => value === "partial").length}</strong><span>Partly correct</span></div><div><strong>{Object.values(statuses).filter((value) => value === "review").length}</strong><span>Needs review</span></div><div><strong>{Object.values(statuses).filter((value) => value === "incorrect").length}</strong><span>Incorrect</span></div></div><div className="answer-review"><h2>Review your answers</h2><div className="progress-rail">{questions.map((question, index) => feedbackByQuestion[question.id] && <button type="button" key={question.id} className={`progress-step is-${statuses[question.id]}`} aria-label={`Review question ${index + 1}: ${statuses[question.id]}`} onClick={() => openReview(question.id)}>{statuses[question.id] === "correct" ? "✓" : statuses[question.id] === "partial" ? "½" : statuses[question.id] === "review" ? "?" : "×"}</button>)}</div></div><div className="question-actions">{(Object.values(statuses).includes("incorrect") || Object.values(statuses).includes("partial")) && <button onClick={reviewMistakes}>Review mistakes</button>}<button className="button-secondary" onClick={() => setPhase("library")}>Choose another chapter</button></div></section>}
