  ...
  return {
    ...
    sources: [{
      ...
      regions: [{
        ...
        runtimeAssetRef: "private://page-1/case-study",
      }],
    }],
    ...
    questions: [{
      ...
      response: { options: [{ id: "a", text: "A" }, { id: "b", text: "B" }] },
      answer: { correctOptionId: "a" },
      rubric: { exact: true },
      hint: "Recall the relevant fact.",
...
describe("question-bank content safety", () => {
  it.each([
    ...
    expect(result.errors).toContain(
      "questions[0] is active but depends on source context that the current question player does not display.",
    );
  });
  ...
  it.each([
    ...
    expect(result.valid).toBe(false);
    expect(result.errors).toContain(
      "questions[0] uses textbook-referential wording; rewrite the prompt as a direct, self-contained question.",
    );
  });
  ...
  it("requires a reason when hidden-context material is held for review", () => {
    ...
    expect(result.valid).toBe(false);
    expect(result.errors).toContain(
      "questions[0] depends on source context and must record reviewReason while it is disabled or under review.",
    );
  });
  ...
  it("allows a source group only when its cited region has a runtime asset", () => {
    const sourceGroup = bankWith("Study the displayed source and answer the questions.", {
      ...
      response: { subquestions: [{ id: "part-a", prompt: "What is shown?" }] },
      answer: { subanswers: [{ id: "part-a", accepted: ["A"] }] },
