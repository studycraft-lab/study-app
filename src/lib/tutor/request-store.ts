...
export async function tutorRequestLibrary(familyId: string, child?: TutorChild) {
  ...
  const progress = (progressResult.data ?? []).flatMap(row => { const pack = allPacks.find(p => p.id === row.pack_id && p.status !== "draft"); return pack ? [{ id: String(row.id), heading: pack.heading, completed: row.state?.phase === "completed" }] : []; });
