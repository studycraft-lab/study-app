alter table public.review_items drop constraint if exists review_items_reason_check;
alter table public.review_items add constraint review_items_reason_check
  check (reason in ('incorrect', 'partial', 'low_confidence', 'maintenance'));
...
begin
  perform 1 from public.study_sessions where id = p_session_id and child_id = p_child_id for update;
  if not found then raise exception 'Study session is unavailable.'; end if;

  select count(*) into v_attempt_count from public.study_attempts where session_id = p_session_id and child_id = p_child_id;
  ...
  with last_imperfect as (
    select question_bank_id, bank_version, question_id, question_version,
      max(attempted_at) filter (where not correct) as last_imperfect_at
    from public.study_attempts where child_id = p_child_id
  ...
  streaks as (
    select a.question_bank_id, a.bank_version, a.question_id, a.question_version,
      count(*) filter (where a.correct and (li.last_imperfect_at is null or a.attempted_at > li.last_imperfect_at))::integer as consecutive_correct
    from public.study_attempts a
    join last_imperfect li using (question_bank_id, bank_version, question_id, question_version)
  ...
  latest as (
    select distinct on (question_bank_id, bank_version, question_id, question_version)
      ...
      correct, earned_marks, max_marks, attempted_at
    from public.study_attempts where child_id = p_child_id
  ...
  rebuilt as (
    select latest.*,
      case
        when not latest.correct and latest.max_marks > 0 and latest.earned_marks / latest.max_marks >= 0.75 then 3
      ...
      case when latest.correct or (latest.max_marks > 0 and latest.earned_marks / latest.max_marks >= 0.75)
        then streaks.consecutive_correct else 0 end as repetitions,
      case when latest.correct then 'maintenance'
        when latest.max_marks > 0 and latest.earned_marks / latest.max_marks >= 0.75 then 'partial'
        else 'incorrect' end as review_reason
    from latest join streaks using (question_bank_id, bank_version, question_id, question_version)
  )
